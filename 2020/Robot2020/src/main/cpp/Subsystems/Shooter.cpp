...
AddChild("Shooter Indexer", shooterIndexer);
    ...
    // Initialize Variables
     ...
     config->GetValueAsDouble("SH_SlowSpeed", m_slowSpeed, 0.25);
     config->GetValueAsDouble("SH_FastSpeed", m_fastSpeed, 0.75);
...
void Shooter::InitDefaultCommand() {
    ...
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND
        ...
        SetDefaultCommand(new ShooterRun(-1));
...
void Shooter::SetShooterSpeedInit(int level) {
	...
	switch (level) {
	case NOCHANGE_SPEED:	// Do not change from current velocity!
		// m_targetVelocityRPM = m_targetVelocityRPM;
		break;
	case STOP_SPEED:
		m_targetVelocityRPM = 0.0;
		break;
	case SLOW_SPEED:
		m_targetVelocityRPM = m_slowSpeed;
		break;
	case FAST_SPEED:
		m_targetVelocityRPM = m_fastSpeed;
		break;
	case SMARTDASH_SPEED:
		m_targetVelocityRPM = frc::SmartDashboard::GetNumber("SH Setpoint", 0.0);
		break;
	default:
...
bool Shooter::SetShooterSpeedIsFinished() {
	...
	if (m_shooterLevel != NOCHANGE_SPEED) {
		if (m_talonValidSH10) {
