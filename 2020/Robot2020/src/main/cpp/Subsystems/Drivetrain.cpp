...
pigeonIMU.reset(new PigeonIMU(20));
    ...
    if (m_talonValidL1)
    {
        ...
        motorL1->ConfigVoltageCompSaturation(12.0, kCANTimeout);
        motorL1->EnableVoltageCompensation(true);

        motorL1->Set(ControlMode::PercentOutput, 0.0);
        motorL1->ConfigSelectedFeedbackSensor(FeedbackDevice::CTRE_MagEncoder_Relative, kPidIndex, kCANTimeout);
        motorL1->SetSensorPhase(false);
        motorL1->SetSelectedSensorPosition(0, kPidIndex, kCANTimeout);
    ...
    if (m_talonValidR3)
    {
        ...
        motorR3->ConfigVoltageCompSaturation(12.0, kCANTimeout);
        motorR3->EnableVoltageCompensation(true);

        motorR3->Set(ControlMode::PercentOutput, 0.0);
        motorR3->ConfigSelectedFeedbackSensor(FeedbackDevice::CTRE_MagEncoder_Relative, kPidIndex, kCANTimeout);
        motorR3->SetSensorPhase(false);
        motorR3->SetSelectedSensorPosition(0, kPidIndex, kCANTimeout);
    ...
    if (m_talonValidR4)
    {
        ...
        motorR4->ConfigVoltageCompSaturation(12.0, kCANTimeout);
...
void Drivetrain::Periodic()
{
    ...
    if (periodicInterval++ % 5 == 0)
    {
        ...
        if (m_talonValidL1)
            encoderLeft = motorL1->GetSelectedSensorPosition(kPidIndex);

        if (m_talonValidR3)
            encoderRight = -motorR3->GetSelectedSensorPosition(kPidIndex);
...
void Drivetrain::VelocityCLDrive(const frc::DifferentialDriveWheelSpeeds &speeds)
{
    ...
    double leftCurSpeed = GetSpeedFPS(motorL1);
    double rightCurSpeed = GetSpeedFPS(motorR3);
...
{
    m_odometry->Update(GetAngle(), units::foot_t(GetDistanceFeet(motorL1)), units::foot_t(GetDistanceFeet(motorR3)));
}

double Drivetrain::GetDistanceFeet(std::shared_ptr<WPI_TalonFX> talon)
{
    return kFeetPerCount * talon->GetSelectedSensorPosition();
}

double Drivetrain::GetSpeedFPS(std::shared_ptr<WPI_TalonFX> talon)
{
    return kFeetPerCount * (talon->GetSelectedSensorVelocity() * 10);
}
...
double Drivetrain::InchesToCounts(double inches)
{
    ...
    counts = (inches / kWheelCircumInches) * kWheelCPR;
    return counts;
}
...
double Drivetrain::CountsToInches(int counts)
{
    ...
    inches = ((double)counts / kWheelCPR) * kWheelCircumInches;
    return inches;
}
...
double Drivetrain::GetEncoderPosition(int motorID)
{
    ...
    case 1: // Left Master
        if (m_talonValidL1)
            ret = motorL1->GetSelectedSensorPosition(kPidIndex);
        break;
    case 3: // Right Master
        if (m_talonValidR3)
            ret = motorR3->GetSelectedSensorPosition(kPidIndex);
        break;
    default:
        break;
    }
...
bool Drivetrain::MoveDriveDistanceMMIsFinished()
{
    ...
    if (m_talonValidL1)
        curCounts_L = motorL1->GetSelectedSensorPosition(kPidIndex);

    if (m_talonValidR3)
        curCounts_R = motorR3->GetSelectedSensorPosition(kPidIndex);
