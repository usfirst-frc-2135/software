...
VisionThread::VisionThread() {
    ...
    std::thread([&] {
        ...
        units::millisecond_t startTime = frc2::GetTime();
        camConnected.SetBoolean(false);

        // Wait up to 3.0 seconds for camera server to detect
        while (!m_cam.IsConnected() && ((frc2::GetTime() - startTime) < 3000_ms)) {
            std::printf("2135: CAM Waiting for camera to connect %d msec\n", (int) (frc2::GetTime() - startTime));
