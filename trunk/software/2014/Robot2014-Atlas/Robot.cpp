...
void Robot::RobotInit() {
	...
	bool newSaveData = false;
	prefs = Preferences::GetInstance();
	if (!prefs->ContainsKey( "AutoDriveDistance" )) {
		printf( "2135: AutoDriveDistance Save\n" );
		prefs->PutDouble( "AutoDriveDistance", 76.0 );
		newSaveData = true;
	}
	if (!prefs->ContainsKey( "PidMaxOutput" )) {
		printf( "2135: PidMaxOutput Save\n" );
		prefs->PutDouble( "PidMaxOutput", 0.7 );
		newSaveData = true;
	}
	if (!prefs->ContainsKey( "SweeperSpeed" )) {
		printf( "2135: SweeperSpeed Save\n" );
		prefs->PutDouble( "SweeperSpeed", 1.0 );
		newSaveData = true;
	}
	if (!prefs->ContainsKey( "ShooterSpeed" )) {
		printf( "2135: ShooterSpeed Save\n" );
...
//		prefs->Save();
	...
	autoChooser->AddObject( "Sit still", new AutoCommand() );
