...
Chassis::Chassis() : Subsystem("Chassis") {
	...
	motorHL = RobotMap::chassisMotorHL;
	motorHR = RobotMap::chassisMotorHR;
	engage = RobotMap::chassisEngage;
	...
	printf("2135: Setting Motor Direction\n");
	...
	motorR5->Set(4);
	printf("2135: Staring Timer\n");
	m_speedTimer = new Timer;
	m_speedTimer->Start();
	printf("2135: PID Parameters\n");
	...
	m_hWheelSpeed = 0.3;
	printf("2135: Chassis Init Finished\n");
}
...
void Chassis::SetHWheelSpeed( double speed )
{
	m_hWheelSpeed = speed;
	printf("2135: SetHWheelSpeed:   %f\n", m_hWheelSpeed);
}

double Chassis::GetHWheelSpeed( void )
{
...
void Chassis::DriveDistanceWithPIDInit( double distance )
{
	...
	leftPID->SetPID(SmartDashboard::GetNumber("L: P", 0),SmartDashboard::GetNumber("L: I", 0),
			SmartDashboard::GetNumber("L: D", 0));
	leftPID->SetOutputRange(m_drivePidSpeedMin, m_drivePidSpeedMax);
	leftPID->SetAbsoluteTolerance(abstolerance);
	encoderL->SetDistancePerPulse(4 * M_PI / 360);

	rightPID->SetPID(SmartDashboard::GetNumber("R: P", 0),SmartDashboard::GetNumber("R: I", 0),
			SmartDashboard::GetNumber("R: D", 0));
...
void Chassis::HWheelEngage( bool engaged )
{
	if (engaged == kHWheelDeploy)
		Robot::chassis->engage->Set(true);
	else
		Robot::chassis->engage->Set(false);
}

void Chassis::HWheelStop( void ) {
	motorHL->Set(0.0);
	motorHR->Set(0.0);
}
