...
Climber::Climber()
{
    ...
    /*std::printf(
        "2135: CL Climber Deployer FWD Solenoid is %s\n",
        (m_climbPosition.IsFwdSolenoidBlackListed()) ? "BLACKLISTED - ERROR" : "OK");
    std::printf(
        "2135: CL Climber Deployer REV Solenoid is %s\n",
        (m_climbPosition.IsRevSolenoidBlackListed()) ? "BLACKLISTED - ERROR" : "OK");
    */

    if (m_climbPosition.IsFwdSolenoidBlackListed())
    {
        spdlog::error("CL Climber Deployer FWD Solenoid is BLACKLISTED");
    }
    else
    {
        spdlog::info("CL Climber Deployer FWD Solenoid is OK");
    }

    if (m_climbPosition.IsRevSolenoidBlackListed())
    {
        spdlog::error("CL Climber Deployer REV Solenoid is BLACKLISTED");
    }
    else
    {
        spdlog::info("CL Climber Deployer REV Solenoid is OK");
    }
    Initialize();
}
