...
void Drivetrain::SimulationPeriodic()
{
    ...
    auto last_update = m_simStateLeft.last_update;
    auto battery_voltage = frc::DriverStation::GetInstance().GetBatteryVoltage();
    auto left_voltage = make_unit<unit_t<volts>>(m_motorL1.GetMotorOutputPercent() * battery_voltage);
    auto right_voltage = make_unit<unit_t<volts>>(m_motorR3.GetMotorOutputPercent() * battery_voltage);

    m_simStateLeft = leftMotorModel(left_voltage, m_simStateLeft);
    m_simStateRight = rightMotorModel(right_voltage, m_simStateRight);

    auto period = std::chrono::duration<unit_t<seconds>>(m_simStateLeft.last_update - last_update).count();

    if (m_simStateLeft.velocity != -m_simStateRight.velocity) {
        // The robot is turning
        auto inside_velocity = std::min(m_simStateLeft.velocity, -m_simStateRight.velocity);
        auto outside_velocity = std::max(m_simStateLeft.velocity, -m_simStateRight.velocity);
        auto turn_radius = -kTrackWidthFeet / 2 * (inside_velocity + outside_velocity) / (inside_velocity - outside_velocity);
        int turn_direction = -frc2135::sgn(m_simStateLeft.velocity + m_simStateRight.velocity);
        auto angular_velocity = (inside_velocity + outside_velocity) / 2 / turn_radius * turn_direction * 1_rad;
        m_simHeading += angular_velocity * period;
    }
    
    m_odometry.Update(frc::Rotation2d(m_simHeading), m_simStateLeft.displacement, -m_simStateRight.displacement);
}
...
void Drivetrain::MoveAlignTurnExecute(frc::XboxController *throttleJstick)
{
    double throttle = 0.0;
