...
namespace frc2135
{
    ...
    void DriverSim::Update(volt_t leftVoltage, volt_t rightVoltage)
    {
        ...
        feet_per_second_squared_t rightAcceleration =
            (rightVoltage - m_kS * sgn(m_currentVelocityRight) - m_kV * m_currentVelocityRight)
            / m_kA;
        ...
        m_currentDisplacementLeft =
            m_currentDisplacementLeft + (m_currentVelocityLeft * elapsedTime);
        m_currentDisplacementRight =
            m_currentDisplacementRight + (m_currentVelocityRight * elapsedTime);

        // Reset the timer
        m_Timer.Reset();
    }

    foot_t DriverSim::getRightDisplacement() { return m_currentDisplacementRight; }

    foot_t DriverSim::getLeftDisplacement() { return m_currentDisplacementLeft; }
