...
void DriverSim::Update(volt_t leftVoltage, volt_t rightVoltage)
{
    ...
    feet_per_second_squared_t leftAcceleration = (leftVoltage - m_kS * sgn(m_currentVelocityLeft) - m_kV * m_currentVelocityLeft) / m_kA;
    feet_per_second_squared_t rightAcceleration = (rightVoltage - m_kS * sgn(m_currentVelocityRight) - m_kV * m_currentVelocityRight) / m_kA;
