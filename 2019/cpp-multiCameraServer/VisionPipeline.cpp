...
#include "frc2135/VisionPipeline.h"
...
VisionPipeline::VisionPipeline() {
	std::printf("2135: VisionPipeline Constructor\n");
}
...
void VisionPipeline::Init() {
	...
	std::printf("2135: VisionPipeline Init\n");
...
void VisionPipeline::Process(cv::Mat inFrame) {

	std::printf("2135: VisionPipeline Process\n");
...
void VisionPipeline::InitializeSmartdashboard(void) {
...
void VisionPipeline::SetCamConfig(cs::UsbCamera cam) {
...
void VisionPipeline::ConfigureCamera(cs::UsbCamera cam, int resWidth, int resHeight, int fps, int bright, int expos) {
...
bool VisionPipeline::DetermineSlant(cv::RotatedRect *rotRect){
	cv::Point2f vert[4];
...
void VisionPipeline::ConvertContoursToBoundingRects(std::vector<std::vector<cv::Point>> *contours, std::vector<tData> *rawData) {
...
void VisionPipeline::ConvertBoundingRectsToValidTargets(std::vector<tData> *rawData, std::vector<tData> *targets) {
	double	score;
	tData t;
...
void VisionPipeline::ConvertValidTargetsToValidHatches(std::vector<tData> *targets, std::vector<tData> *hatches) {
	double 	score;
	tData	h;
...
void VisionPipeline::SortValidHatches(std::vector<tData> *hatches) {
...
void VisionPipeline::PrintTargetData(char name, int idx, tData t) {
	// std::printf("-%c %d: x %d, y %d, w %d, h %d, s %5.1f, d %5.1f, a %5.1f\n", name, idx,
	// 	t.r.x, t.r.y, t.r.width, t.r.height, t.score, t.dist, t.angle);
}

void VisionPipeline::ApplyGridToFrame(cv::Mat frame, pixelRect res/*, double dist, double angle*/) {
	cv::Point	pt1, pt2;
	char	str[32];
...
void VisionPipeline::ApplyRectsToFrame(cv::Mat frame, std::vector<tData> *targets) {
...
void VisionPipeline::ApplyHatchesToFrame(cv::Mat frame, std::vector<tData> *hatches) {
...
void VisionPipeline::ApplyGoalToFrame(cv::Mat frame, tData goal) {
	cv::Point	pt1, pt2;
...
double VisionPipeline::CalcInchesToTarget(double targetWidthInches, cv::Rect rect) {
...
double VisionPipeline::CalcCenteringAngle(double targetWidthInches, cv::Rect rect, double inchesToTarget) {
...
void VisionPipeline::ChooseGoalHatch(std::vector<tData> *hatches, tData *goal) {
	if (!hatches->empty()) {
