...
Elevator::Elevator() : frc::Subsystem("Elevator") {
    ...
    deviceID = motorL7->GetDeviceID();
    error = motorL7->GetLastError();
    if (error != OKAY) {
    	std::printf("2135: ERROR: EL Motor L7 GetDeviceID error - %d\n", error);
    }

    fwVersion = motorL7->GetFirmwareVersion();
    error = motorL7->GetLastError();
    if (error != OKAY) {
    	std::printf("2135: ERROR: EL Motor L7 GetFirmwareVersion error - %d\n", error);
    }

    std::printf("2135: EL Motor L7 ID %d ver %d.%d\n", deviceID, fwVersion/256, fwVersion%256);

    std::printf("2135: EL Motor R8 ID %d ver %d.%d\n", motorR8->GetDeviceID(),
