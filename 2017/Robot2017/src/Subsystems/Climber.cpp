// RobotBuilder Version: 2.0
//
// This file was generated by RobotBuilder. It contains sections of
// code that are automatically generated and assigned by robotbuilder.
// These sections will be updated in the future when you export to
// C++ from RobotBuilder. Do not put any code or make any change in
// the blocks indicating autogenerated code or it will be lost on an
// update. Deleting the comments indicating the section will prevent
// it from being updated in the future.

#include "Climber.h"
#include "../RobotMap.h"
#include "../Commands/ClimberMotorControl.h"

// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INCLUDES
// END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INCLUDES

// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTANTS

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTANTS

Climber::Climber() : Subsystem("Climber") {
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
    motor18 = RobotMap::climberMotor18;
    motor19 = RobotMap::climberMotor19;

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS

    printf("2135: Climber Constructor\n");
    m_lowSpeed = false;
}

void Climber::InitDefaultCommand() {
    // Set the default command for a subsystem here.
    // SetDefaultCommand(new MySpecialCommand());
    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND

        SetDefaultCommand(new ClimberMotorControl(0));

    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DEFAULT_COMMAND
}


// Put methods for controlling this subsystem
// here. Call these from Commands.

void Climber::Initialize(frc::Preferences *prefs)
{
	// Initialize SmartDashboard values - if any
	printf("2135: Climber Initialize\n");
    motor18->ConfigNeutralMode(CAN::TalonSRX::NeutralMode::kNeutralMode_Brake);
	motor18->SetTalonControlMode(ControlMode::SmartControlMode::kPercentVbus);
	motor18->Set(0.0);

    motor19->ConfigNeutralMode(CAN::TalonSRX::NeutralMode::kNeutralMode_Brake);
	motor19->SetTalonControlMode(ControlMode::SmartControlMode::kFollower);
	motor19->Set(18);
	motor19->SetClosedLoopOutputDirection(true);
}

void Climber::UpdateSmartDashboardValues(void)
{

}

void Climber::SetMotorSpeed(int speed)
{
	switch (speed)
	{
	default:
	case CLIMBER_STOP:
		motor18->Set(0.0);
		break;
	case CLIMBER_FORWARD:
		if (m_lowSpeed == true)
			motor18->Set(CLMB_LOW_SPEED_D);
		else
			motor18->Set(CLMB_SPEED_D);
		break;
	case CLIMBER_REVERSE:
		if (m_lowSpeed == true)
			motor18->Set(-CLMB_LOW_SPEED_D);
		else
			motor18->Set(-CLMB_SPEED_D);
		break;
	}
}

void Climber::SetMotorLowSpeed(bool lowSpeed)
{
	m_lowSpeed = lowSpeed;
}
